...
/**
  ----------------------------------------------------------
    DynamicForm v1.44
  ----------------------------------------------------------
    Built Friday, November 13th, 2020, 3:47:05 PM EST
  ----------------------------------------------------------
**/
...
(function (window, document, $, undefined) {
    ...
    var PreSelectOptIn = function () {
        ...
        if (currentCountry && !dynamicForm.options.DisableOptIn) {
    ...
    var DisplayPartnerLegalText = function () {
        ...
        if (partnerName && partnerURL && privacyUrl && optIn) {
            var partnerText = '<div class="form-group" id="partner-legal-text" data-translation-id="partner_text"><p>This is co-hosted in partnership with <a data-translation-group-index="0" target="_blank" href="' + partnerURL + '">' + partnerName + '</a> and Red Hat. As a result, both Red Hat and <a data-translation-group-index="0" target="_blank" href="' + partnerURL + '">' + partnerName + '</a> are collecting your personal ' +
                ...
                'please see: Red Hat’s <a data-translation-group-index="1" target="_blank" href="' + privacyUrl + '">Privacy Statement </a> | ' +
                '<a data-translation-group-index="0" target="_blank" href="' + partnerURL + '">' + partnerName + '</a>&#39s privacy policy</p></div>';
    ...
    dynamicForm.Util = {
        ...
        GetCookieValue: function (name, useRaw) {
            var value = dynamicForm.constants.UNAVAILABLE;
            if (dynamicForm.Util.HasValue(dynamicForm.data[name])) {
                value = dynamicForm.data[name];
            } else {
                if (dynamicForm.Util.HasValue($.cookie(name, undefined, undefined, useRaw))) {
                    value = dynamicForm.data[name] = $.cookie(name, undefined, undefined, useRaw);
                }
            }
            return value;
        },
    ...
    dynamicForm.data = {
        ...
        A_TacticID_Personalization: dynamicForm.constants.UNAVAILABLE,
        A_AdobeMcvisid: dynamicForm.constants.UNAVAILABLE,
        rh_opt_in: '',
    ...
    dynamicForm.BuildOptions = function (opts) {
        ...
        var _sandbox = {
            QA_Version: "1.44 SANDBOX + jQuery " + $.fn.jquery,
            elqSiteId: "711611696"
        };
        var _production = {
            QA_Version: "1.44 + jQuery " + $.fn.jquery,
    ...
    dynamicForm.form = {
        ...
        Long: function (opts) {
            ...
            $.when(dynamicForm.Lookup.Contact()).then(function () {
                ...
                $.when(dynamicForm.Translate(document.getElementById(dynamicForm.options.elqFormContainerId))).then(function () {
                    ...
                    if (!dynamicForm.options.DisableOptIn && (dynamicForm.options.opt_in === "0" || dynamicForm.options.opt_in === dynamicForm.constants.UNAVAILABLE)) {
        ...
        DoSubmit: function () {
            ...
            /**
             * @return {{name: string, value: string}[]} cookie objects array
             */
            function getPersonalizationCookies() {
                var personalizationCookieNames = [
                    {
                        cookieName: 'rh_omni_pc',
                        payloadName: 'A_TacticID_Personalization',
                        value: '',
                        useRaw: false, // does the cookieName contain escaped characters?
                    },
                    {
                        cookieName: 'AMCV_945D02BE532957400A490D4C%40AdobeOrg',
                        payloadName: 'A_AdobeMcvisid',
                        value: '',
                        useRaw: true,
                    }
                ];
                return personalizationCookieNames
                    .map(function (target) {
                        return ({
                            name: target.payloadName,
                            value: dynamicForm.Util.GetCookieValue(target.cookieName, target.useRaw)
                        });
                    });
            }

            var deferred = $.Deferred();
            var dataAsJsonWithoutPersonalizationCookies = $("#" + dynamicForm.options.elqFormName)
                .serializeArray()
            ...
            /*
                There is no guarantee when the personalization cookies are set.
                This block pulls personalization cookies immediately prior to submission
                and appends their value to the values stored in the form.
                Note that this is inconsistent with other patterns which write to a (potentially hidden)
                field then pull the value. 

                Since the cookie values aren't used elsewhere there does not appear to be an advantage to the above pattern
            */
            var dataAsJson = getPersonalizationCookies()
                .reduce(function (accumulator, currentValue) {
                    accumulator[currentValue.name] = currentValue.value;
                    return accumulator;
                }, dataAsJsonWithoutPersonalizationCookies);
